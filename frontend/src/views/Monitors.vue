  ...
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-slate-400">Manage your monitoring endpoints</p>
      </div>
    ...
    <div v-if="store.error" class="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-400 p-4 rounded-lg shadow-sm">
    ...
    <div v-if="store.loading && !store.error" class="flex flex-col items-center justify-center py-12">
      <BaseSpinner size="lg" />
...
const deleteBulkMonitors = async () => {
  ...
  try {
    deleting.value = true
    const promises = Array.from(selectedMonitors.value).map(id => 
...
const handleSubmit = async (data: { name: string; url: string; type: string; interval_seconds: number }) => {
  try {
    ...
    if (e.message?.includes('already exists')) {
      ...
      showError(`Failed to ${editingMonitor.value ? 'update' : 'create'} monitor: ${e.message || 'Unknown error'}`)
    }
  } finally {
